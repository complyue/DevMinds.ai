...
const webappDist = path.join(process.cwd(), 'dist');
...
const PORT = Number(process.env.DMINDS_PORT ?? 5555);
httpServer.listen(PORT, () => {
  console.log(`[devminds] listening on http://localhost:${PORT}`);
  console.log(`[devminds] bearer token = ${DEVMINDS_AUTH_KEY}`);
});

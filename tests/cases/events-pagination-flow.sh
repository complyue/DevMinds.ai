...
mkdir -p "$LOG_DIR"
BE_LOG="$LOG_DIR/backend-eventspage-$(date +%Y%m%d-%H%M%S).log"
FE_LOG="$LOG_DIR/webapp-eventspage-$(date +%Y%m%d-%H%M%S).log"

echo "[e2e:eventspage] root=$ROOT_DIR port=$PORT web_port=$WEB_PORT task=$TASK_ID ws=$TEST_WS_DIR mock=$MOCK_DIR"
echo "[e2e:eventspage] logs -> backend=$BE_LOG webapp=$FE_LOG"
...
ensure_port_free() {
  local port="$1"; local tries=0; local max_tries=30
  ...
  while [[ $tries -lt $max_tries ]]; do
    if [[ -z "$(lsof -ti tcp:$port || true)" ]]; then echo "[e2e] port :$port is free"; break; fi
    tries=$((tries+1)); sleep 0.5
  done
  [[ $tries -ge $max_tries ]] && echo "[e2e][warn] port :$port still busy after wait, proceeding" || true
}
...
start_backend() {
  ensure_port_free "$PORT"; sleep 0.5
...
start_frontend() {
  ensure_port_free "$WEB_PORT"; sleep 0.5
...
# 运行 Playwright 用例
( cd "$ROOT_DIR" && env TASK_ID="$TASK_ID" WEB_BASE="http://localhost:$WEB_PORT" npx -y playwright test tests/e2e/events-pagination.spec.ts ) & PW_PID=$!
wait "$PW_PID"

echo "[e2e:eventspage][ok] 事件分页与日期过滤端到端验证通过"
